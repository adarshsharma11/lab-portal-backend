  ...
  try {
    ...
    if (isFranchise) {
      ...
      if (!data.franchiseId || typeof data.franchiseId !== "string" || !data.franchiseId.trim()) {
        res.status(400).json({ message: "Franchise selection is required for this appointment." });
        return;
      }
    ...
    if (data.patientId && typeof data.patientId === "string" && data.patientId.trim()) {
      ...
      res.status(400).json({ message: "Patient selection is required for this appointment." });
      return;
    }
